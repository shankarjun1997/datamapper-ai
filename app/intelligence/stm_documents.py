...
def build_stm_markdown(sid: str, session: dict) -> str:
    ...
    # ── Data validation (when the mappings were proven against sample data) ──
    dv = session.get("data_validation") or {}
    if dv.get("summary"):
        s = dv["summary"]
        lines.append("## Data Validation")
        lines.append("")
        lines.append(f"Mappings were validated against sample data from "
                     f"**{s.get('tables_with_samples', 0)}** table(s): "
                     f"**{s.get('pass', 0)} pass** · {s.get('warn', 0)} warn · "
                     f"{s.get('fail', 0)} fail · {s.get('no_data', 0)} without samples.")
        issues = [r for r in dv.get("rows", []) if r.get("status") in ("warn", "fail")]
        if issues:
            lines.append("")
            lines.append("| Status | Source Field | Target | Finding |")
            lines.append("|---|---|---|---|")
            for r in issues:
                finding = "; ".join(c["detail"] for c in r.get("checks", [])
                                    if c.get("status") in ("warn", "fail")) or "—"
                lines.append(f"| {r['status']} | {_md_escape(r['src_field'])} "
                             f"| {_md_escape(r['tgt_table'])}.{_md_escape(r['tgt_column'])} "
                             f"| {_md_escape(finding)} |")
        lines.append("")
